...
flavor=m1.tiny
vmargs="--image $image --flavor $flavor --key-name sshkey --user-data cloudinit.sh"

# vm1: net1, net2, floatingip1 on net1 (nic1): OK
nova boot $vmargs --nic net-id=$net1_id --nic net-id=$net2_id vm1
...
# vm2: net2, net1, floatingip2 on net1 (nic2): FAIL
nova boot $vmargs --nic net-id=$net2_id --nic net-id=$net1_id vm2
floatingip2_id=$(floatingip_create $public_id)
floatingip_associate vm2 net1 $floatingip2_id
